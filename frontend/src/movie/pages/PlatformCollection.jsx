...
import { Sparkles, Library, Filter, Languages as LanguagesIcon, ChevronDown, Check, Monitor } from 'lucide-react';
...
import { setPlatformState, updatePlatformList, saveScrollPosition } from '@/store/exploreSlice';
...
const PlatformCollection = () => {
  ...
  const { list: movies, page, totalPages, genre: selectedGenre, lang: selectedLanguage, scrollPos, hasLoaded } = useSelector(state => state.explore.platform);
  ...
  useEffect(() => {
    return () => {
      dispatch(saveScrollPosition({ type: 'platform', position: window.scrollY }));
    };
  }, [dispatch]);
  ...
  const fetchMovies = useCallback(async (pageNum, genreId, langCode) => {
    ...
    setError(null);
    try {
      let endpoint = `${BASE_URL}/discover/movie?api_key=${TMDB_API_KEY}&with_watch_providers=${providerId}&watch_region=IN&sort_by=popularity.desc&page=${pageNum}`;
  ...
  }, [dispatch, providerId, providerName]);

  // Handle Genre/Language changes
  const handleGenreChange = (genreId) => {
    dispatch(setPlatformState({ genre: genreId, page: 1, list: [], hasLoaded: false }));
  };

  const handleLangChange = (langCode) => {
    dispatch(setPlatformState({ lang: langCode, page: 1, list: [], hasLoaded: false }));
  };

  // Initial fetch / trigger when filters change
  useEffect(() => {
    // Reset if providerId changes but hasLoaded is true from previous provider
    if (!hasLoaded) {
      fetchMovies(1, selectedGenre, selectedLanguage);
    }
  }, [selectedGenre, selectedLanguage, fetchMovies, hasLoaded, providerId]);

  // Handle providerId change separately to reset hasLoaded
  useEffect(() => {
    dispatch(setPlatformState({ list: [], page: 1, hasLoaded: false, genre: null, lang: null }));
    ...
    <div className="flex min-h-screen bg-background text-foreground pt-5">
      ...
      <aside className="hidden lg:flex flex-col w-72 sidebar-tonal border-r border-white/5 p-8 gap-8 overflow-y-auto scrollbar-hide">
        ...
        <nav className="flex flex-col gap-1">
          <div className="flex items-center gap-2 text-foreground/40 text-[10px] font-bold uppercase tracking-widest mb-4 mt-6">
            <Filter className="w-3 h-3" /> Filter by Genre
          </div>
          <button
      ...
      <main className="flex-1 px-4 md:px-12 py-12 max-w-[1600px] mx-auto overflow-hidden">
        ...
        <div className="lg:hidden mb-12 space-y-8">
          <h1 className="text-4xl font-black tracking-tightest uppercase">{providerName}</h1>
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="space-y-3">
              <span className="text-[10px] font-black uppercase tracking-widest text-foreground/40 ml-1">Genres</span>
              <ScrollArea className="w-full whitespace-nowrap">
        ...
        <div className="space-y-20">
          {error && (
            <div className="text-center text-destructive py-12 bg-destructive/5 rounded-2xl border border-destructive/10 font-bold">
              {error}
            </div>
          )}
          ...
          <section>
            ...
            >
              {loading ? (
                <div className="flex items-center gap-3 text-primary/40 font-black tracking-widest text-[10px] uppercase">
                  ...
                  <span>Scanning the network...</span>
                </div>
              ) : (
            ...
            {!loading && movies.length === 0 && !error && hasLoaded && (
                <div className="text-center text-muted-foreground py-32 bg-white/5 rounded-3xl border border-white/5">
                    <p className="text-xl font-bold">No movies found in this collection.</p>
                </div>
            )}
          </section>
        </div>
