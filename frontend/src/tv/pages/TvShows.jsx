...
import { Sparkles, Tv, Filter, ArrowRight, Languages as LanguagesIcon, ChevronDown, Check } from 'lucide-react';
...
import { setTVState, updateTVList, saveScrollPosition } from '@/store/exploreSlice';
...
const TvShows = () => {
  ...
  useEffect(() => {
    return () => {
      dispatch(saveScrollPosition({ type: 'tv', position: window.scrollY }));
    };
  }, [dispatch]);
    ...
    <div className="flex min-h-screen bg-background text-foreground pt-5">
      ...
      <main className="flex-1 px-4 md:px-12 py-12 max-w-[1600px] mx-auto overflow-hidden">
        ...
        <div className="lg:hidden mb-12 space-y-8">
          <h1 className="text-4xl font-black tracking-tightest uppercase">Broadcasts</h1>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
              {shows.map((show, index) => {
                ...
                return (
                  <React.Fragment key={`${show.id}-${index}`}>
            ...
            >
              {loading ? (
                <div className="flex items-center gap-3 text-primary/40 font-black tracking-widest text-[10px] uppercase">
                  ...
                  <span>Syncing Narrative Threads...</span>
                </div>
              ) : (
